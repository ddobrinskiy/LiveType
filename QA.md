...
| 54 | Billing screen shows which device the worker recognised, its limit, and the per-device table | **partial — the device line is confirmed on the phone.** Screenshotted on a Pixel 9 reading `This device: default`, which correctly identified why the amounts had gone to $0.00: the phone still held the pre-rename secret while the history had moved to `david`. The cap line and the per-device table have still never rendered — `default` is uncapped and is no longer the owner. See `OPEN_QUESTIONS.md` B10 | yes | | yes |
