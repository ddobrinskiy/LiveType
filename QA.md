...
| 47 | Tapping an indicator shows its status toast — regressed twice, now fixed | build only — the second cause is that a `Toast` **cannot be seen from an IME**: AOSP layers `TYPE_TOAST` at 7 and `TYPE_INPUT_METHOD` at 13, and a text toast is pinned 48dp above the screen's bottom, ~300dp inside a 364dp keyboard; `setGravity` is a no-op at `targetSdk` 35. The label is now a `PopupWindow` — a sub-window of the keyboard's own window, so always drawn above it — with the `Toast` kept as the fallback when no window token is available. Geometry re-derived: the 48dp box is inside every ancestor, and the three children are explicitly inert so the `LOADING` spinner cannot swallow the tap. `assembleDebug` (with `compileDebugKotlin` executing) and `lintDebug` clean, 0 errors, the same 2 pre-existing warnings. Not tapped on a phone: an `ACTION_DOWN` log at the indicator and a first-layout reachability check now separate "the tap never arrived" from "the tap arrived and showed nothing" in logcat | | | yes |
